...
CREATE INDEX IF NOT EXISTS listing_interests_delivery_route_idx
  ON public.listing_interests (delivery_country, delivery_city)
...
BEGIN
  ...
  ELSIF p_kind = 'interest' THEN
    ...
    IF coalesce(v_interest.delivery_phone,'') = '' OR coalesce(v_interest.delivery_address,'') = '' OR coalesce(v_interest.delivery_city,'') = '' OR coalesce(v_interest.delivery_country,'') = '' THEN RAISE EXCEPTION 'The sender must add a phone number and exact destination delivery location in Show interest before acceptance.'; END IF;
    IF coalesce(v_traveler.phone,'') = '' OR coalesce(v_traveler.current_address,'') = '' THEN RAISE EXCEPTION 'The traveler must complete their phone and exact pickup address in their profile before acceptance.'; END IF;
