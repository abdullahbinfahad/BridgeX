...
import { ArrowLeft, Mail, MessageCircle, Plus, RefreshCw, Search, X } from "lucide-react";
...
export default function AdminEnquiries() {
  ...
  const [newSubject, setNewSubject] = useState("BridgeX support");
  const [creating, setCreating] = useState(false);

  useEffect(() => { if (!newContactOpen || memberQuery.trim().length < 2) { setMembers([]); return; } const timer = window.setTimeout(() => { void supabase.from("users").select("id,full_name,email").or(`full_name.ilike.%${memberQuery.trim()}%,email.ilike.%${memberQuery.trim()}%`).order("full_name").limit(20).then(({ data, error }) => { if (error) setNotice(error.message); else setMembers((data ?? []) as Member[]); }); }, 250); return () => window.clearTimeout(timer); }, [memberQuery, newContactOpen]);
  ...
  const createContact = async () => { if (!chosenMember || !newSubject.trim()) return; setCreating(true); const name = chosenMember.full_name || chosenMember.email || "BridgeX member"; const { data, error } = await supabase.from("contact_enquiries").insert({ user_id: chosenMember.id, name, email: chosenMember.email || "", subject: newSubject.trim(), message: "BridgeX Admin opened a private support conversation.", status: "open" }).select("id,user_id,name,email,subject,message,reply_body,status,created_at").single(); setCreating(false); if (error || !data) return setNotice(error?.message || "BridgeX could not create the support conversation."); setRows(current => [data as Enquiry, ...current]); setNewContactOpen(false); setMemberQuery(""); setMembers([]); setChosenMember(null); setNewSubject("BridgeX support"); setSelected(data as Enquiry); };
  ...
  return <main className="min-h-screen bg-[#f7f5ef]/90 px-5 py-8 text-[#172126]"><div className="mx-auto max-w-5xl"><Button onClick={() => go("/admin")} variant="outline" className="rounded-xl bg-white/80 backdrop-blur-md"><ArrowLeft className="mr-2 size-4" />Control panel</Button><div className="mt-7 flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#2d8d62]">Administrator support</p><h1 className="mt-2 font-display text-4xl font-bold">Contact enquiries</h1><p className="mt-2 text-sm text-[#637073]">Live member support conversations, newest message first.</p></div><div className="flex gap-2"><Button onClick={() => setNewContactOpen(true)} className="rounded-xl bg-[#176447]"><Plus className="mr-2 size-4" />New Contact</Button><Button onClick={() => void load()} variant="outline" className="rounded-xl bg-white/80 backdrop-blur-md"><RefreshCw className="mr-2 size-4" />Refresh</Button></div></div><div className="bridgex-glass-panel mt-6 flex h-11 items-center gap-2 rounded-xl border border-white/80 px-3"><Search className="size-4 text-[#647174]" /><Input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search member, subject, or latest message" className="h-auto border-0 bg-transparent p-0 shadow-none focus-visible:ring-0" /></div>{newContactOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-[#172126]/45 p-5"><section className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#2d8d62]">Administrator support</p><h2 className="mt-1 text-xl font-bold">New Contact</h2><p className="mt-2 text-sm text-[#637073]">Search a BridgeX member, then start a private support conversation.</p></div><button onClick={() => setNewContactOpen(false)}><X className="size-5" /></button></div><label className="mt-5 grid gap-2 text-sm font-bold">Search customer<Input autoFocus value={memberQuery} onChange={event => { setMemberQuery(event.target.value); setChosenMember(null); }} placeholder="Name or email" className="h-11 rounded-xl" /></label><div className="mt-3 max-h-48 overflow-y-auto rounded-xl border border-[#172126]/10">{memberQuery.trim().length < 2 ? <p className="p-3 text-sm text-[#637073]">Type at least two characters to search customers.</p> : members.length ? members.map(member => <button key={member.id} onClick={() => setChosenMember(member)} className={`block w-full border-b border-[#172126]/8 px-3 py-3 text-left text-sm last:border-0 ${chosenMember?.id === member.id ? "bg-[#e7f4ea]" : "hover:bg-[#f7f5ef]"}`}><strong>{member.full_name || "BridgeX member"}</strong><span className="ml-2 text-xs text-[#637073]">{member.email}</span></button>) : <p className="p-3 text-sm text-[#637073]">No member found.</p>}</div><label className="mt-4 grid gap-2 text-sm font-bold">Conversation subject<Input value={newSubject} onChange={event => setNewSubject(event.target.value)} className="h-11 rounded-xl" /></label><Button disabled={creating || !chosenMember || !newSubject.trim()} onClick={() => void createContact()} className="mt-5 w-full rounded-xl bg-[#176447]">{creating ? "Creating…" : `Start chat${chosenMember ? ` with ${chosenMember.full_name || chosenMember.email}` : ""}`}</Button></section></div>}{notice && <p className="mt-4 rounded-xl bg-[#edf8f0] px-3 py-2 text-sm font-semibold text-[#176447]">{notice}</p>}<div className="mt-6 grid gap-4">{loading ? <p className="bridgex-glass-panel rounded-2xl p-6">Loading enquiries…</p> : cards.map(({ row, latestBody, latestAt, totalMessages }) => <article key={row.id} className="bridgex-glass-panel rounded-3xl border border-white/80 p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-bold"><Mail className="mr-2 inline size-4 text-[#2d8d62]" />{row.subject}</p><button onClick={() => setSelected(row)} className="mt-2 text-left text-sm font-bold text-[#176447] hover:underline">{row.name}</button><p className="mt-1 text-sm text-[#637073]">{row.email}</p></div><div className="flex items-center gap-2"><span className="inline-flex items-center gap-1 rounded-full bg-[#dff5ea]/85 px-2.5 py-1 text-xs font-bold text-[#176447]"><MessageCircle className="size-3.5" />{totalMessages}</span><span className="rounded-full bg-[#e7f4ea]/85 px-2.5 py-1 text-xs font-bold text-[#176447]">{row.status}</span></div></div><p className="mt-4 text-xs font-bold uppercase tracking-[0.11em] text-[#637073]">Latest message · {messageTime(latestAt)}</p><p className="mt-2 line-clamp-2 whitespace-pre-wrap text-sm leading-6 text-[#526063]">{latestBody}</p><div className="mt-5 flex gap-2"><Button onClick={() => setSelected(row)} size="sm" className="rounded-lg bg-[#176447]">Open live chat with {row.name}</Button></div></article>)}</div></div></main>;
}
