...
export function CreateRequest() {
  ...
  const [form, setForm] = useState({ title: "", serviceType: "Personal item", productCategories: [] as string[], description: "", weight: "", size: "", source: "", budget: "", originCountry: "", purchaseCity: "", destinationCountry: "", destinationCity: "", destinationAddress: "", deliveryDays: "", specialHandling: "None", declaredValue: "", itemPurpose: "", commercialUse: "personal", declarationConfirmed: false });
  ...
  return <PublicLayout><main className="px-5 py-10 lg:px-8 lg:py-14"><div className="mx-auto max-w-[920px]">
    ...
    <form onSubmit={submit} className="mt-8 rounded-3xl border border-[#172126]/8 bg-white p-6 sm:p-8"><div className="grid gap-5">
      ...
      <section className="rounded-2xl border border-[#e4c984] bg-[#fff9ea] p-4"><p className="text-sm font-bold text-[#765b1a]">Truthful item declaration</p><p className="mt-1 text-xs leading-5 text-[#765b1a]">Declare the real value and purpose. Do not label commercial goods as personal luggage or use a false description to avoid a duty, tax, declaration, carrier, or legal requirement.</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><Field label={`Declared item value (${currency})`}><Input required min="0" step="0.01" type="number" inputMode="decimal" value={form.declaredValue} onChange={event => setForm({ ...form, declaredValue: event.target.value })} /></Field><Field label="Item purpose"><Input required value={form.itemPurpose} onChange={event => setForm({ ...form, itemPurpose: event.target.value })} placeholder="Personal use, family use, business sample, resale…" /></Field><Field label="Commercial-use status"><select value={form.commercialUse} onChange={event => setForm({ ...form, commercialUse: event.target.value })} className="h-11 rounded-xl border border-[#d9d7cf] bg-white px-3 text-sm"><option value="personal">Personal / non-commercial</option><option value="commercial">Commercial / resale / business use</option></select></Field></div><label className="mt-4 flex cursor-pointer items-start gap-2 text-xs font-semibold leading-5 text-[#765b1a]"><input required type="checkbox" checked={form.declarationConfirmed} onChange={event => setForm({ ...form, declarationConfirmed: event.target.checked })} className="mt-0.5" />I confirm that the item, quantity, value, purpose, and commercial-use status are truthful. I understand that a false, concealed, substituted, restricted, dangerous, counterfeit, or materially different item may be refused, the order may be paused or cancelled, and records may be shared with competent authorities where required by law.</label></section>
