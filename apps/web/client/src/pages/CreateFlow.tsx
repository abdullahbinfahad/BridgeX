...
import { routeScopeFromCountries } from "@/lib/routeDeclaration";
...
function RouteAwareDeclaration({ international, currency, onCurrencyChange, value, onValueChange, purpose, onPurposeChange, commercialUse, onCommercialUseChange, confirmed, onConfirmedChange }: { international: boolean; currency: string; onCurrencyChange: (value: string) => void; value: string; onValueChange: (value: string) => void; purpose: string; onPurposeChange: (value: string) => void; commercialUse: string; onCommercialUseChange: (value: string) => void; confirmed: boolean; onConfirmedChange: (value: boolean) => void }) {
  if (!international) return <section className="rounded-2xl border border-[#dce7df] bg-[#f6fbf7] p-4"><p className="text-sm font-bold text-[#176447]">Domestic service selected</p><p className="mt-1 text-xs leading-5 text-[#537064]">The international declaration is not required for this same-country route. Your post, categories, item details, and terms acknowledgement must still be accurate and lawful.</p></section>;
  return <section className="rounded-2xl border border-[#e4c984] bg-[#fff9ea] p-4"><p className="text-sm font-bold text-[#765b1a]">Truthful item declaration</p><p className="mt-1 text-xs leading-5 text-[#765b1a]">Declare the real value and purpose. Do not label commercial goods as personal luggage or use a false description to avoid a duty, tax, declaration, carrier, or legal requirement.</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><Field label="Declaration currency"><select value={currency} onChange={event => onCurrencyChange(event.target.value)} className="h-11 rounded-xl border border-[#d9d7cf] bg-white px-3 text-sm">{SUPPORTED_CURRENCIES.map(item => <option key={item.code} value={item.code}>{item.label}</option>)}</select></Field><Field label={`Declared item value (${currency})`}><Input required min="0" step="0.01" type="number" inputMode="decimal" value={value} onChange={event => onValueChange(event.target.value)} /></Field><Field label="Item purpose"><Input required value={purpose} onChange={event => onPurposeChange(event.target.value)} placeholder="Personal use, family use, business sample, resale…" /></Field><Field label="Commercial-use status"><select value={commercialUse} onChange={event => onCommercialUseChange(event.target.value)} className="h-11 rounded-xl border border-[#d9d7cf] bg-white px-3 text-sm"><option value="personal">Personal / non-commercial</option><option value="commercial">Commercial / resale / business use</option></select></Field></div><label className="mt-4 flex cursor-pointer items-start gap-2 text-xs font-semibold leading-5 text-[#765b1a]"><input required type="checkbox" checked={confirmed} onChange={event => onConfirmedChange(event.target.checked)} className="mt-0.5" />I confirm that the item, quantity, value, currency, purpose, and commercial-use status are truthful. I understand that a false, concealed, substituted, restricted, dangerous, counterfeit, or materially different item may be refused, the order may be paused or cancelled, and records may be shared with competent authorities where required by law.</label></section>;
}
...
export function CreateRequest() {
  ...
  const [form, setForm] = useState({ title: "", serviceType: "Personal item", serviceScope: "international", productCategories: [] as string[], description: "", weight: "", size: "", source: "", budget: "", originCountry: "", purchaseCity: "", destinationCountry: "", destinationCity: "", destinationAddress: "", deliveryDays: "", specialHandling: "None", declaredValue: "", declarationCurrency: "BDT", itemPurpose: "", commercialUse: "personal", declarationConfirmed: false });
  const domestic = isDomesticRoute(form.originCountry, form.destinationCountry);
  const routeScope = routeScopeFromCountries(form.originCountry, form.destinationCountry);
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (routeScope !== form.serviceScope) return setMessage("Choose the service type that matches your origin and destination countries. Use Domestic for a same-country route and International for a cross-border route.");
    if (form.serviceScope === "international" && (!form.declaredValue || !form.itemPurpose.trim() || !form.declarationConfirmed)) return setMessage("Declare the item value, currency, purpose, commercial-use status, and truthful-item confirmation before publishing an international request.");
    setBusy(true); setFailed(false); setMessage("");
    try {
      const paths = await uploadMedia(user.id, media);
      const { data, error } = await supabase.from("send_requests").insert({ user_id: user.id, title: form.title.trim(), category: form.serviceType, categories, description: form.description.trim(), weight_kg: Number(form.weight), size_description: form.size.trim() || null, product_link: form.source.trim() || null, image_path: paths[0] ?? null, media_paths: paths, purchase_country: form.originCountry.trim(), purchase_city: form.purchaseCity.trim(), destination_country: form.destinationCountry.trim(), destination_district: form.destinationCountry.trim(), destination_city: form.destinationCity.trim(), destination_address: form.destinationAddress.trim(), delivery_required_days: Number(form.deliveryDays), special_handling: form.specialHandling, budget_bdt: Number(form.budget), currency, service_scope: form.serviceScope, declared_item_value: form.serviceScope === "international" ? Number(form.declaredValue) : null, declared_item_currency: form.serviceScope === "international" ? form.declarationCurrency : null, item_purpose: form.serviceScope === "international" ? form.itemPurpose.trim() : null, declared_commercial_use: form.serviceScope === "international" ? form.commercialUse === "commercial" : null, declaration_confirmed_at: form.serviceScope === "international" ? new Date().toISOString() : null, terms_accepted_at: new Date().toISOString(), terms_version: BRIDGEX_TERMS_VERSION }).select("id").single();
      if (error) throw new Error(error.message);
      await recordAcknowledgement(user.id, "send_request", data.id);
      formElement.reset(); setMedia([]); setAcknowledged(false); setForm({ title: "", serviceType: "Personal item", serviceScope: "international", productCategories: [], description: "", weight: "", size: "", source: "", budget: "", originCountry: "", purchaseCity: "", destinationCountry: "", destinationCity: "", destinationAddress: "", deliveryDays: "", specialHandling: "None", declaredValue: "", declarationCurrency: "BDT", itemPurpose: "", commercialUse: "personal", declarationConfirmed: false });
  ...
  return <PublicLayout><main className="px-5 py-10 lg:px-8 lg:py-14"><div className="mx-auto max-w-[920px]">
    ...
    <form onSubmit={submit} className="mt-8 rounded-3xl border border-[#172126]/8 bg-white p-6 sm:p-8"><div className="grid gap-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2"><Field label="What type of service do you want?"><select value={form.serviceType} onChange={event => setForm({ ...form, serviceType: event.target.value })} className="h-11 rounded-xl border border-[#d9d7cf] bg-[#faf9f5] px-3 text-sm">{SERVICE_TYPES.map(item => <option key={item}>{item}</option>)}</select></Field><Field label="Service route"><select value={form.serviceScope} onChange={event => setForm({ ...form, serviceScope: event.target.value as "domestic" | "international" })} className="h-11 rounded-xl border border-[#d9d7cf] bg-[#faf9f5] px-3 text-sm"><option value="international">International service</option><option value="domestic">Domestic service</option></select></Field><Field label="Purchase country"><Input required value={form.originCountry} onChange={event => setForm({ ...form, originCountry: event.target.value })} placeholder="Any country" /></Field><Field label="Purchase city"><Input required value={form.purchaseCity} onChange={event => setForm({ ...form, purchaseCity: event.target.value })} placeholder="City where the item is purchased or collected" /></Field><Field label="Delivery required within (days)"><Input required min="1" max="365" type="number" value={form.deliveryDays} onChange={event => setForm({ ...form, deliveryDays: event.target.value })} /></Field></div>
      ...
      <RouteAwareDeclaration international={form.serviceScope === "international"} currency={form.declarationCurrency} onCurrencyChange={value => setForm({ ...form, declarationCurrency: value })} value={form.declaredValue} onValueChange={value => setForm({ ...form, declaredValue: value })} purpose={form.itemPurpose} onPurposeChange={value => setForm({ ...form, itemPurpose: value })} commercialUse={form.commercialUse} onCommercialUseChange={value => setForm({ ...form, commercialUse: value })} confirmed={form.declarationConfirmed} onConfirmedChange={value => setForm({ ...form, declarationConfirmed: value })} />
...
export function CreateListing() {
  ...
  return <PublicLayout><main className="px-5 py-10 lg:px-8"><div className="mx-auto max-w-[920px]">
    ...
    <form onSubmit={submit} className="mt-8 rounded-3xl border bg-white p-6"><div className="grid gap-5">
      ...
      <div><div className="flex flex-wrap items-center justify-between gap-2"><b className="text-sm">Categories you accept to carry</b><Button type="button" onClick={selectAll} size="sm" variant="outline" className="rounded-lg bg-white">{categories.length === PRODUCT_CATEGORIES.length ? "Clear all" : "Select all"}</Button></div><div className="mt-2 grid gap-2 rounded-xl border p-3 sm:grid-cols-2">{PRODUCT_CATEGORIES.map(category => <label key={category} className="text-sm"><input type="checkbox" checked={categories.includes(category)} onChange={() => toggleCategory(category)} /> {category}</label>)}</div>{QUANTITY_AWARE_ITEMS.some(item => categories.includes(item)) && <div className="mt-3 grid gap-3 rounded-xl bg-[#f4faf5] p-4 sm:grid-cols-3"><p className="text-sm font-bold sm:col-span-3">Available quantity and optional item budget</p>{QUANTITY_AWARE_ITEMS.filter(item => categories.includes(item)).map(item => <div key={item} className="grid gap-2 rounded-lg border border-[#cfe2d1] bg-white p-3"><p className="text-xs font-bold text-[#263438]">{item}</p><label className="grid gap-1 text-xs font-semibold text-[#526063]">Available quantity<Input type="number" min="1" inputMode="numeric" value={itemQuantities[item] ?? ""} onChange={event => setItemQuantities(current => ({ ...current, [item]: event.target.value }))} placeholder="Optional quantity" /></label><label className="grid gap-1 text-xs font-semibold text-[#526063]">Preferred budget per item ({currency})<Input type="number" min="0" inputMode="decimal" value={itemBudgets[item] ?? ""} onChange={event => setItemBudgets(current => ({ ...current, [item]: event.target.value }))} placeholder="Optional budget" /></label></div>)}</div>}</div>
