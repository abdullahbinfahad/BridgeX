...
export default function InterestPage() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    const payload = { listing_id: listing.id, sender_id: user.id, note: note.trim() || null, status: "pending", categories, quantity_description: quantity.trim() || null, item_quantities: normalizedItemQuantities, weight_kg: Number(weight), total_offer_bdt: Number(offer), delivery_required_by: deliveryRequiredBy };
    ...
    await supabase.from("notifications").insert({ user_id: listing.user_id, actor_id: user.id, type: existing ? "interest_updated" : "interest_received", title: existing ? "Interest updated in your carry space" : "New interest in your carry space", body: `A sender proposed ৳ ${Number(offer).toLocaleString()} for ${weight} kg, needed by ${new Date(`${deliveryRequiredBy}T00:00:00`).toLocaleDateString()}.`, link: "/dashboard/offers", related_id: listing.id });
