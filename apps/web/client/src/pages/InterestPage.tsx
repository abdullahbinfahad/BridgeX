...
export default function InterestPage() {
  ...
  useEffect(() => {
    ...
    void supabase.from("carry_listings").select("id,user_id,origin_country,origin_city,destination_country,destination_city,available_weight_kg,departure_at,estimated_delivery_at,price_bdt,currency,accepted_categories,accepted_item_quantities").eq("id", id).eq("status", "open").maybeSingle().then(({ data }) => { const nextListing = data as Listing | null; setListing(nextListing); setDeliveryCountry(nextListing?.destination_country ?? ""); setDeliveryCity(nextListing?.destination_city ?? ""); setLoading(false); });
  }, [id]);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    const payload = { listing_id: listing.id, sender_id: user.id, note: note.trim() || null, status: "pending", categories, quantity_description: quantity.trim() || null, item_quantities: normalizedItemQuantities, weight_kg: Number(weight), total_offer_bdt: Number(offer), currency, delivery_required_by: deliveryRequiredBy, delivery_recipient_name: deliveryRecipientName.trim() || null, delivery_phone: deliveryPhone.trim(), delivery_address: deliveryAddress.trim(), delivery_city: deliveryCity.trim(), delivery_country: deliveryCountry.trim() };
