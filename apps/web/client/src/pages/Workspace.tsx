...
function Offers() {
  ...
  return <ListPage title="Incoming offers and interests" copy="Review every real response to your posts here. There are no sample offers in this workspace." action="Post an item" href="/create-request"><div className="grid gap-5">{message && <p className="rounded-xl bg-[#f4f2eb] px-3 py-2 text-sm font-semibold text-[#536164]">{message}</p>}{loading ? <p className="rounded-2xl bg-white p-6 text-sm font-semibold text-[#647174]">Loading responses…</p> : offers.length || interests.length ? <><section><h2 className="mb-3 text-sm font-bold uppercase tracking-[0.12em] text-[#617073]">Offers on my item requests</h2><div className="grid gap-3">{offers.length ? offers.map(item => responseCard("offer", item)) : <p className="rounded-2xl bg-white p-5 text-sm text-[#637073]">No traveler offers yet.</p>}</div></section><section><h2 className="mb-3 text-sm font-bold uppercase tracking-[0.12em] text-[#617073]">Interest in my carry space</h2><div className="grid gap-3">{interests.length ? interests.map(item => responseCard("interest", item)) : <p className="rounded-2xl bg-white p-5 text-sm text-[#637073]">No sender interest yet.</p>}</div></section></> : <EmptyContent copy="Post an item request or carry space, then incoming offers and interests will appear here." href="/marketplace" label="Open marketplace" />}</div></ListPage>;
}
