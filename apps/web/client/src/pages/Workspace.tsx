...
function Offers() {
  ...
  const update = async (table: "offers" | "listing_interests", id: string, status: "accepted" | "rejected") => { if (!user) return; setMessage(""); const response = table === "offers" ? offers.find(item => item.id === id) : interests.find(item => item.id === id); const { error } = await supabase.from(table).update({ status, updated_at: new Date().toISOString() }).eq("id", id); if (error) { setMessage(error.message); return; } if (response) { const isOffer = table === "offers"; const recipientId = isOffer ? (response as OwnerOffer).traveler_id : (response as OwnerInterest).sender_id; const relatedId = isOffer ? (response as OwnerOffer).request_id : (response as OwnerInterest).listing_id; await supabase.from("notifications").insert({ user_id: recipientId, actor_id: user.id, type: `${table}_${status}`, title: status === "accepted" ? "Your response was accepted" : "Your response was declined", body: status === "accepted" ? "The post owner accepted your response. Open BridgeX to continue the protected matching process." : "The post owner declined this response. You can browse other live posts in the marketplace.", link: "/dashboard/offers", related_id: relatedId }); } setMessage(status === "accepted" ? "Response accepted. Continue with the protected matching process when both sides confirm." : "Response declined."); await load(); };
...
const content = section === "verification" ? <GlobalVerification /> : section === "wallet" ? <Wallet data={data} /> : section === "orders" ? <Orders data={data} /> : section === "offers" ? <Offers /> : section === "settings" ? <SettingsView /> : section === "requests" ? <MyRequests data={data} /> : section === "listings" ? <MyListings data={data} /> : section === "reviews" ? <Reviews /> : <Overview data={data} />;
