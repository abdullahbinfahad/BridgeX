...
const SettlementCard = ({ payment }: { payment: PaymentProof }) => payment.settlement_currency && payment.settlement_amount !== null && payment.exchange_rate !== null ? <div className="mt-6 rounded-2xl border border-[#a8d7bb] bg-[#f1faf4] p-4 text-sm leading-6 text-[#25553a]"><p className="font-bold">Platform settlement conversion</p><p className="mt-1">{money(payment.amount, payment.currency)} × {Number(payment.exchange_rate).toLocaleString(undefined, { maximumFractionDigits: 8 })} = <strong>{money(payment.settlement_amount, payment.settlement_currency)}</strong></p><p className="mt-1 text-xs text-[#4d715d]">Rate published {payment.exchange_rate_updated_at ? new Date(payment.exchange_rate_updated_at).toLocaleString() : "with this payment"}. This payment record keeps this rate even if an administrator publishes a newer daily rate.</p></div> : <div className="mt-6 rounded-2xl border border-[#e4c984] bg-[#fff9ea] p-4 text-sm leading-6 text-[#765b1a]"><strong>Settlement rate unavailable.</strong><p className="mt-1">An administrator must publish the current {payment.currency} to CNY rate before this payment can be started.</p></div>;
