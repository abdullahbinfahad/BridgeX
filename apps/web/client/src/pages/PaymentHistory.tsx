...
type PaymentProof = { id: string; reference: string; response_kind: "offer" | "interest"; amount: number; currency: string; status: string; payment_method: "alipay" | "wechat_pay" | null; payer_reference: string | null; payer_note: string | null; submitted_at: string | null; reviewer_note: string | null; created_at: string };
...
export default function PaymentHistory({ mode = "payments" }: { mode?: "payments" | "payouts" }) {
  ...
  const load = async () => {
    ...
    const [paymentResult, payoutResult, profileResult] = await Promise.all([
      supabase.from("bridgex_payment_proofs").select("id,reference,response_kind,amount,currency,status,payment_method,payer_reference,payer_note,submitted_at,reviewer_note,created_at").eq("payer_id", user.id).order("created_at", { ascending: false }).limit(100),
  ...
  if (routeType === "record") {
    ...
    return <><PageHeader eyebrow="Payments" title={selectedPayment?.reference || "Loading payment"} copy="Review the protected payment record and complete any remaining action." backHref="/dashboard/payments" />{message}<section className="mt-7 rounded-3xl border border-[#172126]/8 bg-white p-6">{loading || !selectedPayment ? <p className="rounded-xl bg-[#f5f3ed] p-4 text-sm text-[#637073]">Loading payment record…</p> : <><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#2d8d62]">Protected payment record</p><h2 className="mt-1 font-display text-2xl font-bold">{money(selectedPayment.amount, selectedPayment.currency)}</h2><p className="mt-2 text-sm text-[#637073]">{selectedPayment.response_kind === "offer" ? "Traveler offer" : "Carry-space interest"} · created {new Date(selectedPayment.created_at).toLocaleString()}</p></div><Badge className={tone(selectedPayment.status)}>{label(selectedPayment.status)}</Badge></div>{["pending_payment", "rejected"].includes(selectedPayment.status) ? <><div className="mt-6 grid gap-3 sm:grid-cols-2"><button onClick={() => setMethod("alipay")} className={`rounded-2xl border p-4 font-bold ${method === "alipay" ? "border-[#1677ff] bg-[#eef6ff] text-[#1677ff]" : "border-[#172126]/10"}`}>Alipay</button><button onClick={() => setMethod("wechat_pay")} className={`rounded-2xl border p-4 font-bold ${method === "wechat_pay" ? "border-[#09bb07] bg-[#edfaef] text-[#078d06]" : "border-[#172126]/10"}`}>WeChat Pay</button></div><div className="mt-5 rounded-2xl border border-[#e4c984] bg-[#fff9ea] p-4 text-sm leading-6 text-[#765b1a]"><strong>Use the exact displayed amount.</strong><p className="mt-1">Add a readable payment screenshot and the transfer reference if available.</p>{selectedPayment.status === "rejected" && selectedPayment.reviewer_note && <p className="mt-2 rounded-lg bg-white/75 p-2 font-semibold">Review note: {selectedPayment.reviewer_note}</p>}</div><div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="grid gap-2 text-sm font-bold">Transfer reference (optional)<input value={reference} onChange={event => setReference(event.target.value)} className={nativeInput} placeholder="Reference in your payment app" /></label><label className="grid gap-2 text-sm font-bold">Payment screenshot<span className="flex h-11 cursor-pointer items-center rounded-xl border border-[#172126]/12 bg-[#fbfaf7] px-3 text-sm"><FileUp className="mr-2 size-4" />{proofFile?.name || "Choose screenshot"}<input className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" onChange={event => setProofFile(event.target.files?.[0] ?? null)} /></span></label></div><label className="mt-4 grid gap-2 text-sm font-bold">Note for review (optional)<Textarea value={note} onChange={event => setNote(event.target.value)} className="min-h-20 rounded-xl" /></label><Button onClick={() => void submitProof()} disabled={submitting || !proofFile} className="mt-5 rounded-xl bg-[#176447] font-bold">{submitting ? "Submitting…" : "Submit payment screenshot"}</Button></> : <div className={`mt-6 rounded-2xl p-5 text-sm ${selectedPayment.status === "verified" ? "bg-[#dff5ea] text-[#176447]" : "bg-[#fff7e5] text-[#805700]"}`}><strong>{selectedPayment.status === "verified" ? "Payment verified" : "Payment verifying"}</strong><p className="mt-1">{selectedPayment.status === "verified" ? "Your protected deal details are now available." : "Your evidence is awaiting review."}</p>{selectedPayment.status === "verified" && <Link href="/dashboard/deals"><Button className="mt-4 rounded-lg bg-[#176447]">Open protected chat</Button></Link>}</div>}</>}</section></>;
  }
