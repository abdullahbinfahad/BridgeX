...
export default function PostDetail() {
  ...
  return <PublicLayout><main className="px-5 py-10 lg:px-8 lg:py-14"><div className="mx-auto max-w-5xl">
    ...
    <div className="mt-7 grid gap-7 lg:grid-cols-[1.15fr_0.85fr]"><section>
      ...
      <div className="mt-6 grid gap-3 text-sm leading-6 text-[#536164]"><p className="flex gap-2"><Plane className="mt-0.5 size-4 shrink-0 text-[#2d8d62]" />{route}</p><p className="flex gap-2"><Weight className="mt-0.5 size-4 shrink-0 text-[#2d8d62]" />{request ? `${post.weight_kg} kg · Size: ${post.size_description || "Not stated"} · Budget ${money(post.budget_bdt)}` : `${post.available_weight_kg} kg available · ${money(post.price_bdt)} / ${post.pricing_mode === "per_kg" ? "kg" : "item"}`}</p>{request && <><p className="flex gap-2"><Clock3 className="mt-0.5 size-4 shrink-0 text-[#2d8d62]" />Delivery required within: <strong>{post.delivery_required_days ? `${post.delivery_required_days} days` : "Not stated"}</strong></p><p className="flex gap-2"><PackageCheck className="mt-0.5 size-4 shrink-0 text-[#2d8d62]" />Special handling: <strong>{post.special_handling || "None stated"}</strong></p></>}<p className="flex gap-2"><CalendarDays className="mt-0.5 size-4 shrink-0 text-[#2d8d62]" />Published {new Date(post.created_at).toLocaleString()}</p>{post.destination_address && <p className="flex gap-2"><MapPin className="mt-0.5 size-4 shrink-0 text-[#2d8d62]" />Exact destination address is protected until a trusted match is selected.</p>}</div>
      <div className="mt-7 rounded-3xl border border-[#172126]/8 bg-white p-6"><h2 className="text-lg font-bold">Post details</h2>{request && categories.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{categories.map((category: string) => <Badge key={category} className="bg-[#fff4d2] text-[#7b5d09] hover:bg-[#fff4d2]">{category}</Badge>)}</div>}<p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-[#526063]">{request ? post.description : post.notes || "No additional notes were provided."}</p>{request && post.product_link && <a href={post.product_link} target="_blank" rel="noreferrer" className="mt-4 inline-flex text-sm font-bold text-[#176447]">Open product link</a>}</div>
