...
import { ArrowLeft, CalendarDays, Clock3, ImageIcon, MapPin, MessageSquareMore, PackageCheck, Plane, ShieldCheck, Star, UserRound, Weight } from "lucide-react";
...
export default function PostDetail() {
  ...
  return <PublicLayout><main className="px-5 py-10 lg:px-8 lg:py-14"><div className="mx-auto max-w-5xl">
    ...
    <div className="mt-7 grid gap-7 lg:grid-cols-[1.15fr_0.85fr]"><section>
      <div className="flex flex-wrap items-center gap-2"><Badge className={request ? "bg-[#e7f4ea] text-[#176447]" : "bg-[#172126] text-white"}>{request ? "Item request" : "Carry space"}</Badge><span className="rounded-full bg-[#f0eee7] px-2.5 py-1 text-xs font-bold text-[#607073]">{post.status}</span></div>
      ...
      <div className="mt-6 grid gap-3 text-sm leading-6 text-[#536164]"><p className="flex gap-2"><Plane className="mt-0.5 size-4 shrink-0 text-[#2d8d62]" />{route}</p><p className="flex gap-2"><Weight className="mt-0.5 size-4 shrink-0 text-[#2d8d62]" />{request ? `${post.weight_kg} kg · Size: ${post.size_description || "Not stated"} · Budget ${money(post.budget_bdt)}` : `${post.available_weight_kg} kg available · ${money(post.price_bdt)} / ${post.pricing_mode === "per_kg" ? "kg" : "item"}`}</p>{request ? <><p className="flex gap-2"><Clock3 className="mt-0.5 size-4 shrink-0 text-[#2d8d62]" />Delivery required within: <strong>{post.delivery_required_days ? `${post.delivery_required_days} days` : "Not stated"}</strong></p><p className="flex gap-2"><PackageCheck className="mt-0.5 size-4 shrink-0 text-[#2d8d62]" />Special handling: <strong>{post.special_handling || "None stated"}</strong></p></> : <><p className="flex gap-2"><Plane className="mt-0.5 size-4 shrink-0 text-[#2d8d62]" />Transport: <strong>{post.transport_mode === "cargo" ? "Cargo" : "Flight / personal carry"}</strong></p><p className="flex gap-2"><CalendarDays className="mt-0.5 size-4 shrink-0 text-[#2d8d62]" />Departure: <strong>{post.departure_at ? new Date(post.departure_at).toLocaleString() : "Not stated"}</strong></p><p className="flex gap-2"><Clock3 className="mt-0.5 size-4 shrink-0 text-[#2d8d62]" />Estimated delivery: <strong>{post.estimated_delivery_at ? new Date(post.estimated_delivery_at).toLocaleString() : "Not stated"}</strong></p></>}<p className="flex gap-2"><CalendarDays className="mt-0.5 size-4 shrink-0 text-[#2d8d62]" />Published {new Date(post.created_at).toLocaleString()}</p>{post.destination_address && <p className="flex gap-2"><MapPin className="mt-0.5 size-4 shrink-0 text-[#2d8d62]" />Exact destination address is protected until a trusted match is selected.</p>}</div>
      <div className="mt-7 rounded-3xl border border-[#172126]/8 bg-white p-6"><h2 className="text-lg font-bold">{request ? "Post details" : "Carry-space details"}</h2>{request && categories.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{categories.map((category: string) => <Badge key={category} className="bg-[#fff4d2] text-[#7b5d09] hover:bg-[#fff4d2]">{category}</Badge>)}</div>}{!request && <><div className="mt-4 grid gap-3 rounded-2xl bg-[#f5f3ed] p-4 text-sm sm:grid-cols-2">{post.airline_name && <p><strong>Airline:</strong> {post.airline_name}</p>}{post.flight_number && <p><strong>Flight number:</strong> {post.flight_number}</p>}{post.cargo_provider && <p><strong>Cargo provider:</strong> {post.cargo_provider}</p>}{post.cargo_reference && <p><strong>Cargo reference:</strong> {post.cargo_reference}</p>}{!post.airline_name && !post.flight_number && !post.cargo_provider && !post.cargo_reference && <p className="sm:col-span-2 text-[#637073]">No additional transport reference was provided.</p>}</div><div className="mt-5"><p className="text-sm font-bold">Accepted categories</p>{acceptedCategories.length ? <div className="mt-2 flex flex-wrap gap-2">{acceptedCategories.map((category: string) => <Badge key={category} className="bg-[#e7f4ea] text-[#176447] hover:bg-[#e7f4ea]">{category}</Badge>)}</div> : <p className="mt-2 text-sm text-[#637073]">No category restrictions were stated.</p>}</div>{acceptedItemQuantities.length > 0 && <div className="mt-5"><p className="text-sm font-bold">Available quantities</p><div className="mt-2 grid gap-2 sm:grid-cols-3">{acceptedItemQuantities.map(([item, quantity]) => <div key={item} className="rounded-xl bg-[#f4faf5] px-3 py-2 text-sm"><strong>{item}</strong><span className="ml-2 text-[#176447]">{String(quantity)} available</span></div>)}</div></div>}</>}<p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-[#526063]">{request ? post.description : post.notes || "No additional notes were provided."}</p>{request && post.product_link && <a href={post.product_link} target="_blank" rel="noreferrer" className="mt-4 inline-flex text-sm font-bold text-[#176447]">Open product link</a>}</div>
