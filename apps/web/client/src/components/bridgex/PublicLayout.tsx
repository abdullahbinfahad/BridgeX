...
function AccountMenu({ mobile = false }: { mobile?: boolean }) {
  ...
  const paymentLink = <button onClick={() => go("/dashboard/payments", "payments")} className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-bold hover:bg-[#f0ede5]"><CircleDollarSign className="size-4" />Payment history{updateBadge(updates.payments)}</button>;
