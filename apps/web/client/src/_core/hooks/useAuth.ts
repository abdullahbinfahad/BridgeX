...
type MemberProfile = { full_name?: string | null; phone?: string | null; bio?: string | null; avatar_path?: string | null; current_country?: string | null; current_city?: string | null; current_address?: string | null; home_country?: string | null; home_city?: string | null; home_address?: string | null; china_address?: string | null; preferred_currency?: string | null; preferred_language?: string | null; restriction_reason?: string | null };
type BridgeXUser = { id: string; email?: string; name?: string; avatarUrl?: string; user_metadata?: Record<string, unknown>; profile?: MemberProfile; role?: "member" | "admin" | "super_admin"; verificationStatus?: "not_submitted" | "pending_review" | "approved" | "rejected"; onboardingComplete?: boolean; suspended?: boolean } | null;
...
async function readBridgeXUser(): Promise<BridgeXUser> {
  ...
  const { data: profile, error: profileError } = await supabase.from("users").select("role,verification_status,onboarding_complete,suspended,restriction_reason,full_name,phone,bio,avatar_path,current_country,current_city,current_address,home_country,home_city,home_address,china_address,preferred_currency,preferred_language").eq("id", authenticatedUser.id).maybeSingle();
  ...
  return { ...authenticatedUser, name, avatarUrl: uploadedAvatarUrl || providerAvatar, profile: profile ?? undefined, role: profile?.role === "super_admin" ? "super_admin" : profile?.role === "admin" ? "admin" : "member", verificationStatus: profile?.verification_status ?? "not_submitted", onboardingComplete: Boolean(profile?.onboarding_complete), suspended: Boolean(profile?.suspended) };
}
