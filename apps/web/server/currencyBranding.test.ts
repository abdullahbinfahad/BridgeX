...
describe("BridgeX currency, cargo, support, and brand release safeguards", () => {
  ...
  it("uses the supplied logo for shared web branding, browser favicon, and mobile app configuration", () => {
    ...
    expect(read("apps/mobile/app.json")).toContain('"versionCode": 15');
  });

  it("uses the independent native app shell in the BridgeX Android 1.4.0 release", () => {
    ...
    expect(read("apps/mobile/android/app/build.gradle")).toContain("versionCode 15");
    expect(read("apps/mobile/android/app/build.gradle")).toContain('versionName "1.4.0"');
  });
  ...
  it("uses the completed independent-native Android v1.4.0 APK link alongside the Windows download entry", () => {
    const layout = read("apps/web/client/src/components/bridgex/PublicLayout.tsx");
    expect(layout).toContain("wd0I0lEt6WSxDGHye9k0qhzVsFsKSSJuqnUAgzjG5ik.apk");
  ...
  it("uses concise download links and keeps a separate signed Google Play App Bundle profile", () => {
    ...
    expect(appConfig).toContain('"versionCode": 15');
