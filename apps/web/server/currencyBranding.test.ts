...
describe("BridgeX currency, cargo, support, and brand release safeguards", () => {
  ...
  it("uses the supplied logo for shared web branding, browser favicon, and mobile app configuration", () => {
    ...
    expect(read("apps/mobile/app.json")).toContain('"versionCode": 16');
  });
  ...
  it("uses the independent native app shell in the BridgeX Android 1.5.0 release", () => {
    ...
    expect(read("apps/mobile/android/app/build.gradle")).toContain("versionCode 16");
    expect(read("apps/mobile/android/app/build.gradle")).toContain('versionName "1.5.0"');
  });
  ...
  it("uses concise download links and keeps a separate signed Google Play App Bundle profile", () => {
    ...
    expect(appConfig).toContain('"versionCode": 16');
    ...
    expect(gradle).toContain('versionName "1.5.0"');
