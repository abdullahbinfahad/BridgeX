...
function readAuthCallback(url: string) {
  ...
  return { access_token, refresh_token, code: values.get("code") };
}
...
export function AuthScreen({ onGuest }: Props) {
  ...
  const signInWithGoogle = () => run(async () => {
    ...
    const sessionResult = callback.code ? await supabase.auth.exchangeCodeForSession(callback.code) : callback.access_token && callback.refresh_token ? await supabase.auth.setSession({ access_token: callback.access_token, refresh_token: callback.refresh_token }) : { error: new Error("Google sign-in finished without a secure session. Check that the BridgeX mobile redirect URL is allowed in Supabase Auth.") };
