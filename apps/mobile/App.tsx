...
export default function App() {
  const webView = useRef<WebView>(null); const registeredUsers = useRef(new Set<string>()); const initialPageReady = useRef(false); const loadingTimer = useRef<ReturnType<typeof setTimeout> | null>(null); const [loading, setLoading] = useState(true); const [error, setError] = useState<string | null>(null); const [pushStatus, setPushStatus] = useState("");
  const stopLoading = () => { if (loadingTimer.current) clearTimeout(loadingTimer.current); loadingTimer.current = null; initialPageReady.current = true; setLoading(false); };
  const startInitialLoading = () => { if (initialPageReady.current) return; setLoading(true); setError(null); if (loadingTimer.current) clearTimeout(loadingTimer.current); loadingTimer.current = setTimeout(() => { initialPageReady.current = true; setLoading(false); }, 7000); };
  const retry = () => { initialPageReady.current = false; setError(null); startInitialLoading(); webView.current?.reload(); };
