...
const BRIDGEX_URL = `${BRIDGEX_ORIGIN}/?app=android&build=11`;
...
const MOBILE_COMPATIBILITY_BRIDGE = `
  (function () {
    ...
    window.__BRIDGEX_ANDROID_WRAPPER__ = true;
    try { window.sessionStorage.setItem('bridgex-android-wrapper', 'true'); window.localStorage.setItem('bridgex-android-wrapper', 'true'); } catch (_) {}
    var hideBridgeXBack = function () {
      var style = document.getElementById('bridgex-android-back-control-style');
      if (!style) { style = document.createElement('style'); style.id = 'bridgex-android-back-control-style'; style.textContent = '.bridgex-global-back,[data-bridgex-global-back="true"]{display:none!important;visibility:hidden!important;pointer-events:none!important;}'; (document.head || document.documentElement).appendChild(style); }
      var controls = document.querySelectorAll('.bridgex-global-back,[data-bridgex-global-back="true"]');
      Array.prototype.forEach.call(controls, function (control) { control.setAttribute('aria-hidden', 'true'); control.setAttribute('tabindex', '-1'); control.style.setProperty('display', 'none', 'important'); });
    };
    hideBridgeXBack();
    document.addEventListener('DOMContentLoaded', hideBridgeXBack);
    if (window.MutationObserver) { new MutationObserver(hideBridgeXBack).observe(document.documentElement, { childList: true, subtree: true }); }
